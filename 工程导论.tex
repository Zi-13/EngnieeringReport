...
\begin{tikzpicture}
    ...
    % —————— 1. 欧美方案（均衡型） ——————
    % 定义三个截距点 (X, Y, Z)
    % 欧美：硬件(0.5)中等，农艺(0.9)极高，互信(0.9)极高
    \addplot3[
        patch,
        patch type=triangle,
        opacity=0.3,
    ...
    % 添加图例条目（透明）
    \addlegendimage{area legend, blue!30, fill=blue!30}
    \addlegendentry{欧美方案（生物/互信优势）}

    % —————— 2. 中国方案（非对称型） ——————
    % 中国：硬件(1.1)极强（降维打击），农艺(0.3)弱，互信(0.2)弱
    \addplot3[
        patch,
        patch type=triangle,
        opacity=0.7,
    ...
    \addlegendimage{area legend, red!60, fill=red!60}
    \addlegendentry{中国方案（工程极致，基底薄弱）}
    
    % —————— 3. 辅助线与标注 ——————
    % 标注中国方案的尖端
    \node[red, anchor=west] at (axis cs:1.1, 0, 0) {极致性价比与敏捷性};
    % 标注欧美方案的高点
    \node[blue, anchor=south] at (axis cs:0, 0, 0.9) {地缘安全壁垒};
    \node[blue, anchor=west] at (axis cs:0, 0.9, 0) {百年农艺数据};

    \end{axis}
\end{tikzpicture}
\caption{中国智慧农业国际竞争力三维能力流形图 (3D Capability Manifold)}
\label{fig:3d_manifold}
\end{figure}

\textbf{图解分析：}
\begin{itemize}
    \item \textbf{红色曲面（中国）：} 向X轴极度延伸，代表在工程制造、算法迭代上的极致优势；但在Y轴（农艺）和Z轴（互信）呈现塌陷状，覆盖体积小，表明系统鲁棒性受限于基础学科与国际规则。
    \item \textbf{蓝色曲面（欧美）：} 虽在X轴不及中国敏捷，但在YZ轴拥有深厚壁垒，形成了一个更均衡、抗风险能力更强的立体结构。
\end{itemize}
